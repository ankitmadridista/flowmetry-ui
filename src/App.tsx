...
function AccessDenied() {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "var(--text)", opacity: 0.6, margin: 0 }}>
...
function NavBar() {
  ...
  return (
    <nav className="app-nav">
      {/* 2. Updated Brand click to go to /dashboard instead of / */}
      <span
      ...
      <div className="nav-links-desktop">
        {showDashboard && (
          // 3. Cleaned up active state logic since dashboard is now cleanly at /dashboard
          <button
      ...
      {menuOpen && (
        <div className="nav-mobile-menu">
          ...
          {showInvoices && (
            <button
              className={`nav-mobile-link${isActive("/invoices") ? " active" : ""}`}
              onClick={() => go("/invoices")}
            >
              Invoices
            </button>
          ...
          {showCustomers && (
            <button
              className={`nav-mobile-link${isActive("/customers") ? " active" : ""}`}
              onClick={() => go("/customers")}
            >
              Customers
            </button>
          ...
          {showSecurity && (
            <button
              className={`nav-mobile-link${isActive("/security") ? " active" : ""}`}
              onClick={() => go("/security")}
            >
              Security
            </button>
...
function ProtectedLayout() {
  ...
  return (
    <PermissionProvider>
      <NavBar />
      <Routes>
        {/* 4. Moved Dashboard from / to /dashboard */}
        <Route
          path="/dashboard"
          element={
        ...
        />
        {/* 5. Fallback for protected routes goes to dashboard */}
...
export default function App(): React.JSX.Element {
  ...
  return (
    <Routes>
      {/* 6. Root route is now the Landing Page (or redirects to dashboard if logged in) */}
      <Route
        path="/"
        element={
          isAuthenticated ? (
